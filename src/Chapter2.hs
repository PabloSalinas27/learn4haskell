...
-}
subList :: Int -> Int -> [a] -> [a]
subList prim sec list 
  |  0 > prim || prim > sec = []
  | otherwise = take (sec - prim +1) $ drop prim list
...
Number of bits needed -1 for is representation in binary, although it is
 problably more due to having to represent its lenght etc
...
-}
takeEven :: [a] -> [a]
takeEven i = reverse $ go []  i False
  where
    go :: [a] -> [a] -> Bool -> [a]
    go acc [] _ = acc
    go acc (_:xs) True = go acc xs False
    go acc (x:xs) False = go (x:acc) xs True
...
-- smartReplicate ls = foldr (\ l -> (++) (replicate l l)) [] ls
-- smartReplicate [] = []
...
rewind :: [a] -> [a]
rewind [] = []
-- rewind x = drop molenght  x ++ rewind (take molenght x)
--   where
--     molenght = length x -1
